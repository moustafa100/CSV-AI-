    ...
    if uploaded_file is not None :
        ...
        try:
            ...
            texts=text_splitter.split_documents(data)
             ...
             loader=CSVLoader(file_path=tmp_file_path,encoding="cp122")
             data=loader.load()
             texts=text_splitter.split_documents(data)
...
def analyze (temperature,model_name):
    ...
    if uploaded_file is not None:
        ...
        if 'generated' not in st.session_state:
            st.session_state['generated']=["Hello ! Ask me anything about Documnet 🤗"]
        ...
        with container:
            with st.form(key="my_form",clear_on_submit=True):
                user_input=st.text_input("Use CSV agent for precise information about the structure of your csv file:",placeholder="e-g : how many rows in my files ?:",key='input')
                submit_button=st.form_submit_button(label='Send')
            if submit_button and user_input:
